...
class APICalculatorController extends AbstractController
{
    ...
    #[Route('/multiplication', name: 'api_calculator_multiplication')]
    #[Methode("POST")]
    public function multiplication(Request $request, CalculatorService $calculator): Response
    {
        $data = json_decode($request->getContent(), true);
        $value1 = intval($data['value1']);
        $value2 = intval($data['value2']);
        return $this->json(['value' => $calculator->multiplication($value1, $value2)]);
    }
